...
from als_assistant.services.pv_finder.core import PVSearchResult

# Observability and tracing disabled for benchmarking
tracer = None

# Import utilities from benchmark_utils.py
from als_assistant.benchmarks.pv_finder.benchmark_utils import (
    load_query_database,
    evaluate_result,
    generate_analysis
)
...
def get_model_configuration() -> Dict[str, str]:
    ...
    try:
        ...
        keyword_config = get_model_config("pv_finder_keyword")
...
def setup_model_override(provider: str, model_id: str):
    ...
    try:
        ...
        # Create the override function (new API: model_name, config_path)
        def override_get_model_config(model_name: str, config_path: str = None):
            # Only override PV finder model configs
            if model_name.startswith("pv_finder_"):
                return {
                    "provider": provider,
                    "model_id": model_id,
                    "max_tokens": 4096
            ...
            return setup_model_override._original_get_model_config(model_name, config_path)
