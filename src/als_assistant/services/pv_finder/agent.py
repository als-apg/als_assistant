...
# langfuse ##########################
from als_assistant.utils import configure_langfuse
...
from als_assistant.services.pv_finder.util import format_pv_results_for_span
...
# Lazy initialization to avoid registry access at module import time
_pv_query_agent = None

def _get_pv_query_agent():
...
async def _get_dynamic_system_prompt_impl(ctx: RunContext[QueryAgentDeps]) -> str:
...
async def _validate_pvs_impl(ctx: RunContext[QueryAgentDeps], result: PVQueryOutput) -> PVQueryOutput:
