...
from als_assistant.database.ao import ao_config
...
def get_collection(auto_load=True, hard_reset=False, validate=False):
    ...
    try:
        ...
        # Force connection check to fail fast if MongoDB is unavailable
        client.server_info()
    except Exception as e:
        raise ConnectionError(
            f"Cannot connect to MongoDB at {ao_config.DB_HOST}:{ao_config.DB_PORT}. "
            f"Is the MongoDB container running? Start it with: podman-compose up -d mongo. "
            f"Error: {str(e)}"
        ) from e
...
def _load_ao_from_files():
    ...
    from als_assistant.database.ao.data.MML_ao_250413_SR import MML_ao_SR
    from als_assistant.database.ao.data.MML_ao_250413_BR import MML_ao_BR
    from als_assistant.database.ao.data.MML_ao_250413_GTL import MML_ao_GTL
    from als_assistant.database.ao.data.MML_ao_250413_LN import MML_ao_LN
    from als_assistant.database.ao.data.MML_ao_250413_LTB import MML_ao_LTB
    from als_assistant.database.ao.data.MML_ao_250413_BTS import MML_ao_BTS
