...
def configure_langfuse(langfuse_host: str = None):
    ...
    if not langfuse_enabled:
        # Configure logfire with minimal settings (no actual sending)
        # This prevents auto-configuration errors
        try:
            logfire.configure(
                service_name='als-assistant-agents',
                send_to_logfire=False,
            )
            logger.debug("Configured logfire with disabled mode")
        except Exception as e:
            logger.warning(f"Error configuring logfire: {e}. Using basic tracer.")
        # Return a regular tracer but don't configure any exporters
