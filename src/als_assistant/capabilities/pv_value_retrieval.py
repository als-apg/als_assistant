...
from als_assistant.context_classes import PVValues, PVValue

if TYPE_CHECKING:
    pass

logger = get_logger("pv_value_retrieval")
...
try:
    from als_assistant.database.PVs.pv_names_list import pv_names
except ImportError:
...
async def read_pv_with_connector(connector, pv_address: str, timeout: Optional[float] = None) -> Dict[str, Any]:
    """
    Reads a live EPICS Process Variable (PV) value and metadata using a provided connector.

    Args:
        connector: Control system connector instance (reused across multiple calls)
        pv_address: The address of the Process Variable to read.
        timeout (optional): Timeout in seconds for each attempt. If None, uses connector default.
    ...
    try:
        # Use connector API to read PV (replaces direct epics.PV() usage)
        pv_result = await connector.read_pv(pv_address, timeout=timeout)

        # Extract value and units from connector result
        value = pv_result.value
        units = pv_result.metadata.units or ''  # Get units, default to empty string
        logger.debug(f"Retrieved PV {pv_address}: value={value}, units='{units}'")

        # Return structured result with value and units (same format as before)
        return {
        ...
        # Map connector TimeoutError to our EPICSTimeoutError
        raise EPICSTimeoutError(f"Timeout reading PV {pv_address}: {str(e)}")
    except ConnectionError as e:
        # Map connector ConnectionError to our EPICSGatewayError
        error_msg = str(e).lower()
        if "gateway" in error_msg or "connect" in error_msg or "host" in error_msg:
            raise EPICSGatewayError(f"Gateway/connection error reading PV '{pv_address}': {str(e)}")
        else:
            raise EPICSChannelAccessError(f"Connection error reading PV '{pv_address}': {str(e)}")
    except Exception as e:
        # For all other errors, use the general channel access error
        error_msg = str(e).lower()
...
class PVValueRetrievalCapability(BaseCapability):
    ...
    async def execute(state: AgentState, **kwargs) -> Dict[str, Any]:
        ...
        streamer = get_streamer("pv_value_retrieval", state)
        ...
        # Create connector ONCE for all PV reads (efficient reuse)
        connector = await ConnectorFactory.create_control_system_connector()

        try:
            # Get all PV values using the same connector
            pv_values = {}
            ...
            for i, pv_address in enumerate(pv_addresses_context.pvs):
                ...
                # Pass connector to helper function (reused for efficiency)
                pv_result = await read_pv_with_connector(connector, pv_address)
            ...
            # Always disconnect connector after all PVs are read
