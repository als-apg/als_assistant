...
.. seealso::
   :mod:`als_assistant.services.pv_finder.agent` : Core PV finder functionality
...
# Validate configuration early before importing modules that depend on it
...
try:
    ...
    if db_config is None or not isinstance(db_config, dict) or not db_config.get("database_name"):
        logger.error("ERROR: Database configuration is missing or invalid!")
        logger.error("Expected: database.ao_db.database_name to be set in config.yml")
        sys.exit(1)
    
    logger.info(f"✓ Database config validated: {db_config.get('database_name')}")
...
try:
    from als_assistant.services.pv_finder.agent import run_pv_finder_graph
    from als_assistant.services.pv_finder.util import initialize_nltk_resources
    logger.info("Successfully imported pv_finder modules")
except Exception as e:
    logger.error(f"Error importing pv_finder modules: {e}", exc_info=True)
    raise
...
try:
    ...
    logger.error(f"Error initializing NLTK: {e}", exc_info=True)
    raise
...
async def run_pv_finder(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Exception in run_pv_finder: {e}", exc_info=True)
